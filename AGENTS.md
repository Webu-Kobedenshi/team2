...
- ユーザーの意見は重要な入力として扱うが、そのまま合わせず、目的、利用状況、実装コスト、保守性を踏まえて判断する。
- 違和感の共有を受けた場合も、すぐ変更に進まず、必要に応じて選択肢、推奨案、リスクを示す。
- UI/UX 相談では、見た目の好み、ユーザー体験、実装範囲を分けて考える。
- 変更前に、現在の Issue の完了条件に含まれるか、後続 Issue へ分けるべきかを確認する。
- デザイン相談では、「足す」だけでなく「削る」「後続へ回す」選択肢も検討する。

...
export function Result({ commonPoints, onRestart }: ResultProps) {
  return (
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-12">
          {commonPoints.length > 0 ? (
            commonPoints.map((item, index) => (
              <div key={index} className="rounded-3xl bg-sky-50 p-8 shadow">
                ...
                <p className="mt-4 text-xl text-slate-700">
                  全員の答え：
                  <span className="font-bold text-sky-600">{item.answer}</span>
                </p>
              </div>
            ))
          ) : (

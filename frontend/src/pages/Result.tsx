...
export function Result() {
  ...
  const { processedPlayers, analysisMap } = useMemo(() => {
    if (!gameSession || !gameSession.answers) {
      return { processedPlayers: [], analysisMap: new Map() };
    }
    ...
    players.forEach((currentPlayer, currentIndex) => {
      const currentRawAnswer = gameSession.answers[currentIndex].answers || {};

      players.forEach((otherPlayer, otherIndex) => {
        if (currentIndex === otherIndex) return; // 自分自身とは比較しない

        const otherRawAnswer = gameSession.answers[otherIndex].answers || {};
    ...
    // 3. 各プレイヤーの「総共通点数」と「相性抜群（最高カウント）の相手」を計算
  ...
  // 初期表示時に最初のプレイヤーを自動選択
  useEffect(() => {
    if (processedPlayers.length > 0 && !selectedPlayerId) {
      setSelectedPlayerId(processedPlayers[0].id);
    }
  }, [processedPlayers, selectedPlayerId]);

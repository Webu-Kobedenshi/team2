...
export function Result() {
  ...
  const { processedPlayers, analysisMap } = useMemo(() => {
    ...
    const players: ProcessedPlayer[] = answers.map((_ans, index) => {
      return {
        id: `p_${index}`,
        name: `プレイヤー ${index + 1}`,
    ...
    players.forEach((currentPlayer, currentIndex) => {
      const currentRawAnswer = answers[currentIndex].answers;
      players.forEach((otherPlayer, otherIndex) => {
        if (currentIndex === otherIndex) return; // 自分自身とは比較しない

        const otherRawAnswer = answers[otherIndex].answers;
            ...
            <div className="grid gap-3">
              ...
              {processedPlayers.map((player) => {
                ...
                const isSelected =
                  ...
                  (!selectedPlayerId && player.id === processedPlayers[0]?.id);
                return (
                  <button
                    key={player.id}
